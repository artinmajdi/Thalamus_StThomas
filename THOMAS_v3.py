...
def main(args, temp_path, pool):
    ...
    print '--- Performing jointfusion. --- Elapsed: %s' % timedelta(seconds=time.time() - t)
    ...
    pool.map(partial(label_fusion_picsl, input_image, atlas_images),
        [dict(
            ...
            output_label=os.path.join(jfusion_path, label+'.nii.gz'),
   ...
   # else:
    ...
    # print '--- Performing antsjointfusion. --- Elapsed: %s' % timedelta(seconds=time.time() - t)
    ...
    files_jf = [(os.path.join(jfusion_path, label + '.nii.gz'), os.path.join(output_path, label + '.nii.gz')) for label in labels]
    ...
    # Resort output to original ordering for jfusion
    pool.map(parallel_command,
        ['%s %s %s %s' % (os.path.join(this_path, 'swapdimlike.py'), in_file, orig_input_image, out_file) for in_file, out_file in files_jf])
    # # Resort output to original ordering for antsjointfusion
    # pool.map(parallel_command,
...
if __name__ == '__main__':
    ...
    if args.tempdir:
        ...
        jfusion_path = os.path.join(temp_path,'jfusion')
        if not os.path.exists(jfusion_path):
            print 'Making %s' % os.path.abspath(jfusion_path)
            #os.makedirs(temp_path)
            os.makedirs(jfusion_path)
    else:
