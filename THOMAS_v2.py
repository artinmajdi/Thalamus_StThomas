...
from v1_functions import estimate_crop, crop_target_and_priors, warp_priors
...
def main(args, temp_path, pool):
    ...
    print '--- Estimating thalamus crop. --- Elapsed: %s' % timedelta(seconds=time.time() - t)
    # TODO don't use global var pool_small
    crop_mask, affines = estimate_crop(subjects, input_image, output_path=temp_path, pool=pool_small)
    print '--- Cropping thalamus from target and priors. --- Elapsed: %s' % timedelta(seconds=time.time() - t)
    # TODO allow padding to be adjustable
    target, priors, mask = crop_target_and_priors(input_image, crop_mask, affines, output_path=temp_path, pool=pool)
    ...
    warped_labels = warp_priors(target, priors, mask, labels, output_path=temp_path, pool=pool)
